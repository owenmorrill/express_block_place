...
namespace Drupal\express_block_place\Plugin\DisplayVariant;
 ...
 *
 * @PageDisplayVariant(
 *   id = "express_block_place_page",
 *   admin_label = @Translation("Page with blocks and place block buttons")
 * )
 */
class ExpressBlockPlaceBlockPageVariant extends BlockPageVariant {
  ...
  public function build() {
    ...
    foreach ($visible_regions as $region => $region_name) {
      ...
      $operations['block_description'] = [
        ...
        '#context' => [
          'link' => Link::createFromRoute($title, 'express_block_place.admin_library', ['theme' => $theme_name], [
            'query' => $query,
            'attributes' => [
              'title' => $title,
    ...
    $build['#attached']['library'][] = 'express_block_place/drupal.express_block_place';
    return $build;
  }
